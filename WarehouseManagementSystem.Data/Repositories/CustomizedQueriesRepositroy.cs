...
namespace WarehouseManagementSystem.Data.Repositories
{
    public class CustomizedQueriesRepositroy
    {
        ...
        public async Task<List<WarehouseItemsStateWithTransfer>> GetAllItemsAtWarehouseWithSupplierTransferAsync(int warehouseId)
        {
            ...
            var receipts = from rvd in context.ReceiptVoucherDetails
                           ...
                           where rv.WarehouseId == warehouseId
            ...
            var transferIn = from tvd in context.TransferVoucherDetails
                             ...
                             join ii in context.InventoryItems
                             on new { tvd.ItemCode, WarehouseId = tv.ToWarehouseId, tvd.ProductionDate, tvd.ExpiryDate } equals new { ii.ItemCode, ii.WarehouseId, ii.ProductionDate, ii.ExpiryDate }
                             join item in context.Items on ii.ItemCode equals item.Code
                             where tv.ToWarehouseId == warehouseId
                             select new WarehouseItemsStateWithTransfer
                             {
                                 ItemName = item.Name,
                                 ItemCode = ii.ItemCode,
            ...
            var transferOut = from tvd in context.TransferVoucherDetails
                              join tv in context.TransferVouchers on tvd.VoucherId equals tv.Id
                              join ii in context.InventoryItems
                              on new { tvd.ItemCode, WarehouseId = tv.FromWarehouseId, tvd.ProductionDate, tvd.ExpiryDate } equals new { ii.ItemCode, ii.WarehouseId, ii.ProductionDate, ii.ExpiryDate }
                              join item in context.Items on ii.ItemCode equals item.Code
                              where tv.FromWarehouseId == warehouseId
                              select new WarehouseItemsStateWithTransfer
                              {
                                  ItemName = item.Name,
                                  ItemCode = ii.ItemCode,
            ...
            var issues = from ivd in context.IssueVoucherDetails
                         ...
                         join ii in context.InventoryItems
                         on new { ivd.ItemCode, iv.WarehouseId, ivd.ProductionDate, ivd.ExpiryDate } equals new { ii.ItemCode, ii.WarehouseId, ii.ProductionDate, ii.ExpiryDate }
                         join item in context.Items on ii.ItemCode equals item.Code
        ...
        public async Task<List<WarehouseItemsStateWithTransfer>> GetAllItemsAtWarehouseWithinTimeRangeAsync(int warehouseId, DateOnly start, DateOnly end)
        {
            ...
            var receipts = from rvd in context.ReceiptVoucherDetails
                           ...
                           new { rvd.ItemCode, rv.WarehouseId, rvd.ProductionDate, rvd.ExpiryDate } equals new { ii.ItemCode, ii.WarehouseId, ii.ProductionDate, ii.ExpiryDate }
                           join item in context.Items on ii.ItemCode equals item.Code
                           join s in context.People.OfType<Supplier>() on rv.SupplierId equals s.Id
                           where rv.WarehouseId == warehouseId
                           && rv.Date >= start
        ...
        #region Remaining days for expiration
